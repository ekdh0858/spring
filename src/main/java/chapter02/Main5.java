...
public class Main5 {
	...
	public static void main(String[] args) {
		// 스프링 컨테이너는 설정 파일(클래스) 엥 있는 @Autowired 애노테이션도 분석하고
		// 설정 파일에 있는 빈 객체의 클래스도 분석해서 @Autowired 에노테이션을 찾아서
		// 자동으로 의존 주입을 해줌
		// 설정 클래스를 빈 객체로 등록
...
//		ctx.getBean(AppConf1.class); 
		//구조에 따라서는 이름을 안적어도됨, 꼭 빈의 이름을 써야하는건 아님
		// 특정 탕비의 빈이 컨테이너 안에 1개만 있다면
		// 빈의 타입만 써도 그러한 타입의 빈을 가져옴
		// 컨테이너에 해당하는 타입의 빈이 없다면 NoSuchBeanDefinitionException 예외가 발생
		// 컨테이너에 해당하는 타입의 빈이 여러개면 NoUniqueDefinitionException 예외가 발생
